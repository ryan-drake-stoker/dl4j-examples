 ...
 * @author Alex Black
 */
public class VariableRatioSentimentExampleIterator implements DataSetIterator {
    ...
    private int pos_neg_splitter = 2;
     ...
     */
    public VariableRatioSentimentExampleIterator(String dataDirectory, WordVectors wordVectors, int batchSize, int truncateLength, boolean train, int pos_neg_ratio) throws IOException {
        ...
        pos_neg_splitter = pos_neg_ratio + 1;
    }
    ...
    private int posReviewNumberFor(int num){
        return ((int) Math.floor(num / pos_neg_splitter)) + 1;
    }

    private int negReviewNumberFor(int num){
        return num - posReviewNumberFor(num);
    }
    ...
    public String loadReviewToString(int index) throws IOException{
        File f;
        if(index % pos_neg_splitter == 0) f = positiveFiles[posReviewNumberFor(index)];
        else f = negativeFiles[negReviewNumberFor(index)];
        return FileUtils.readFileToString(f);
    }

    /** Convenience method to get label for review */
    public boolean isPositiveReview(int index){
        return index % pos_neg_splitter == 0;
    }
